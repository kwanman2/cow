...
double advance( double pi[][N2+2*NGhost][NPR], double pb[][N2+2*NGhost][NPR], 
	double Dt, double pf[][N2+2*NGhost][NPR],double unew[][N2+2*NGhost][NPR],double dt2) ;
...
void step_ch()
{
	...
	//fprintf(stderr,"h") ;
	ndt = advance(p, p,dt, ph1,uh,1./6.*dt) ;
	ndt = advance(p, p, 0.25*dt, ph4,uh,0.);
        ndt = advance(ph4, ph1, 0.25*dt, ph2,uh,1./6.*dt) ;
	ndt = advance(p, ph2, dt, ph3,uh,2./3.*dt);
	//fprintf(stderr,"f") ;
