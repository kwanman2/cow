...
FTYPE thindiskrhopow=-1.3; // closer to thick disk // jane1: SUPERMADNEW # -1 so Qmri constant vs. radius.      //tom: donut disk index 
...
int pre_init_specific_init(void)
{
  ...
  if(WHICHPROBLEM==RADDONUT){
    if(RADNT_DONUTTYPE==DONUTTHINDISK||RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      h_over_r=0.3; // jane:SUPERMADNEW     //tom: keep this
...
int init_global(void)
{
  ...
  //  cour=0.8;
  cour=0.49999; // 0.8 is too unstable for RADBEAM2D with curved flow relative to grid.     //tom:courant factor
...
//    }
 ...
 //   a=0.0; // no spin in case use MCOORD=KSCOORDS
    ...
    DTr = 200; //number of time steps for restart dumps
    tf = 2000*DTdumpgen[0]; // 100 dumps(?)
 ...
 //   a=0.0; // no spin in case use MCOORD=KSCOORDS
  ...
  if(WHICHPROBLEM==RADNT || WHICHPROBLEM==RADFLATDISK || WHICHPROBLEM==RADDONUT || WHICHPROBLEM==RADCYLBEAM || WHICHPROBLEM==RADCYLBEAMCART){
    ...
    TIMEORDER=2; // faster and sufficient       //tom

    RADNT_DONUTTYPE=DONUTTHINDISK2; // jane: add this here SUPERMADNEW  //tom
    ...
    /////////////////////////////////
    // DONUT selections
    RADNT_DONUTTYPE=DONUTTHINDISK2; // jane: disk configuration. SUPERMADNEW        //tom
    //RADNT_DONUTTYPE=DONUTOLEK;
    //RADNT_DONUTTYPE=DONUTOHSUGA;
  ...
  if(WHICHPROBLEM==RADDONUT){
    if(RADNT_DONUTTYPE==DONUTTHINDISK||RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      ...
      //      h_over_r=0.1;
      h_over_r=0.3;// jane: thick disk initial h/r. SUPERMADNEW     //tom
      h_over_r_jet=2.0*h_over_r;
    }
    ...
    if(RADNT_DONUTTYPE==DONUTTHINDISK || RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      RADDONUT_OPTICALLYTHICKTORUS=1; // otherwise, pressure only from gas.
      if(RADDONUT_OPTICALLYTHICKTORUS==1) gamtorus=4.0/3.0; // then should be as if gam=4/3 so radiation supports torus properly at t=0     //tom
      else gamtorus=gam;
      ...
      if(1){// SUPERMADNEW
        ...
        if(a==0.8 && FIELDTYPE==FIELDJONMAD){  //Jane: this spin
          ...
          RADNT_RHODONUT/=1E5;//jane:rescale density when Mbh increases from 10 to 10^6 
		  RADNT_RHODONUT/=(10.0);//jane:change from 10^6 to 10^7
		  ...
		  */ 
          ...
          RADNT_RHODONUT*=(4.4); // Mdot\sim 135Ledd/c^2                                        
          RADNT_RHODONUT*=(3.75); // Mdot\sim 135Ledd/c^2                                       
          RADNT_RHODONUT/=(143); // Mdot\sim 5Mdotedd  
		  */
		  //RADNT_RHODONUT=5.97228858098423E-05;	//Tom: result of the above calculations, try to get Mdot\sim 5Mdotedd
		  trifprintf("After Conditionals: RADNT_RHODONUT=%21.15g\n",RADNT_RHODONUT);
        }
    ...
    ///////////////////////////////////
    // DONUT atmosphere:
    RADNT_ROUT=2.0; // jane: what radius ATMMIN things are defining
    ...
    if(1){ // SUPERMADNEW
      RADNT_RHOATMMIN=RADNT_RHODONUT*4E-5; //jane: increase atmosphere magnetic field       //tom: what's this
    }
    ...
    else cooling=KORAL;                 //tom: radiative ransfer scheme
    ...
    if(WHICHPROBLEM==RADDONUT && totalsize[1]>=64){     //tom
    ...
    tf = 1e5; //jane: Time      //tom

    if(DOWALDDEN) tf=1e5;
	
	if(tf<=2e4){
      DTr = 50; //Tom: set lower dumping frequency if tf is short 
    }
...
int init_defcoord(void)
{
 ...
 //   a=0.0; // no spin in case use MCOORD=KSCOORDS
    ...
    if(MCOORD==KSCOORDS){
      RADBONDI_MINX=1.5; // few cells inside horizon // 1.9 good for N1=512
      RADBONDI_MAXX=2e4;
    }
    else{
      RADBONDI_MINX=2.5;
  ...
  if(WHICHPROBLEM==RADDONUT){
    ...
    if(DOWALDDEN==0) defcoord=JET6COORDS;       //tom: keep this
    else if(DOWALDDEN){
    ...
    if(a==0.8){
      ...
      //      setRin_withchecks(&Rin);
        ...
        Rin = floor(Rhor) + ceil((Rhor - floor(Rhor)) * 10) / 10 - 0.3; //tom
    }
...
int init_grid_post_set_grid(FTYPE (*prim)[NSTORE2][NSTORE3][NPR], FTYPE (*pstag)[NSTORE2][NSTORE3][NPR], FTYPE (*ucons)[NSTORE2][NSTORE3][NPR], FTYPE (*vpot)[NSTORE1+SHIFTSTORE1][NSTORE2+SHIFTSTORE2][NSTORE3+SHIFTSTORE3], FTYPE (*Bhat)[NSTORE2][NSTORE3][NPR], FTYPE (*panalytic)[NSTORE2][NSTORE3][NPR], FTYPE (*pstaganalytic)[NSTORE2][NSTORE3][NPR], FTYPE (*vpotanalytic)[NSTORE1+SHIFTSTORE1][NSTORE2+SHIFTSTORE2][NSTORE3+SHIFTSTORE3], FTYPE (*Bhatanalytic)[NSTORE2][NSTORE3][NPR], FTYPE (*F1)[NSTORE2][NSTORE3][NPR+NSPECIAL], FTYPE (*F2)[NSTORE2][NSTORE3][NPR+NSPECIAL], FTYPE (*F3)[NSTORE2][NSTORE3][NPR+NSPECIAL], FTYPE (*Atemp)[NSTORE1+SHIFTSTORE1][NSTORE2+SHIFTSTORE2][NSTORE3+SHIFTSTORE3])
{
  ...
  if(WHICHPROBLEM==RADDONUT){
    ...
    if(RADNT_DONUTTYPE==DONUTTHINDISK || RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      ...
      rinfield=Risco;
      //      beta=1E30;
      beta=60.0; //jane: where to set the beta 
    ...
    if(RADNT_DONUTTYPE==DONUTTHINDISK || RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      ...
      //      beta=1E30;
      if(FIELDTYPE==FIELDJONMAD){
        beta=30.0; // so for MAD will have ~1 mri wavelength per half-height H      //tom:cow
      }
      else{
...
#if(WHICHPROBLEM==RADDONUT)                             //tom: opacity
...
#define WHICHRADATM 0 // 0,1,2,3
  ...
  if(WHICHPROBLEM==RADNT || WHICHPROBLEM==RADFLATDISK || WHICHPROBLEM==RADDONUT){
    ...
    *whichvel=VEL3; // use 3-velocity since later set donut using VEL3
    ...
    if(1){ // SUPERMADNEW jane: modify gas atmosphere gradient
      int set_fieldtype(void);
      int FIELDTYPE=set_fieldtype();
      if(FIELDTYPE==FIELDJONMAD){
        // so bsq/rho isn't super high at large radii with current choice for magnetic field
        pr[RHO]=RADNT_RHOATMMIN*pow(r/RADNT_ROUT,-1.1); // SUPERMADNEW: jane: change the index back to 1.1      //tom: atmosphere
      }
    }
    else{
    ...
    pr[UU]=RADNT_UINTATMMIN*pow(r/RADNT_ROUT,-2.5);     //tom: atmosphere
    ...
    if(1){
      ...
      if(PRAD0>=0){
        ...
        pradfforthoatm[PRAD0]=RADNT_ERADATMMIN*pow(r/RADNT_ROUT,-2.5);      //tom: initial radiation energy density profile ???
...
// analytical solution for RADDONUT donut           //tom tom: radiative disc solution
...
//tom notes: fieldtype: FIELDJONMAD, DISK2FIELD, TOROIDALFIELD, OLEKFIELD, VERTFIELD, DISK2VERT, OHSUGAFIELD, NOFIELD, BLANDFORDQUAD, SPLITMONOPOLE, MONOPOLE, FIELDWALD
int set_fieldtype(void)
{
  int FIELDTYPE;
  ...
  if(WHICHPROBLEM==RADDONUT){
    ...
    if(RADNT_DONUTTYPE==DONUTTHINDISK || RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      ...
      FIELDTYPE=FIELDJONMAD; // SUPERMADNEW     //tom
...
//tom: this is only for FIELDTYPE== SPLITMONOPOLE || MONOPOLE || FIELDWALD, refer to init_postvpot
...
//tom: it seems only RADCYLJET uses this
