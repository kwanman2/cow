...
FTYPE thindiskrhopow=-1.3; // closer to thick disk // jane1: SUPERMADNEW # -1 so Qmri constant vs. radius.
...
//    }
 ...
 //   a=0.0; // no spin in case use MCOORD=KSCOORDS
  ...
  if(WHICHPROBLEM==RADNT || WHICHPROBLEM==RADFLATDISK || WHICHPROBLEM==RADDONUT || WHICHPROBLEM==RADCYLBEAM || WHICHPROBLEM==RADCYLBEAMCART){
    ...
    RADNT_DONUTTYPE=DONUTTHINDISK2; // jane: add this here SUPERMADNEW
    ...
    /////////////////////////////////
    // DONUT selections
    RADNT_DONUTTYPE=DONUTTHINDISK2; // jane: disk configuration. SUPERMADNEW
    //RADNT_DONUTTYPE=DONUTOLEK;
    //RADNT_DONUTTYPE=DONUTOHSUGA;
  ...
  if(WHICHPROBLEM==RADDONUT){
    if(RADNT_DONUTTYPE==DONUTTHINDISK||RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      ...
      //      h_over_r=0.1;
      h_over_r=0.3;// jane: thick disk initial h/r. SUPERMADNEW
      h_over_r_jet=2.0*h_over_r;
    }
    ...
    if(RADNT_DONUTTYPE==DONUTTHINDISK || RADNT_DONUTTYPE==DONUTTHINDISK2||RADNT_DONUTTYPE==DONUTTHINDISK3){
      ...
      if(1){// SUPERMADNEW
        ...
        if(a==0.5 && FIELDTYPE==FIELDJONMAD){    //jane: not calling this spin
    ...
    if(1){ // SUPERMADNEW
      RADNT_RHOATMMIN=RADNT_RHODONUT*4E-5; //jane: increase atmosphere magnetic field
    }
    ...
    *whichvel=VEL3; // use 3-velocity since later set donut using VEL3
    ...
    if(1){ // SUPERMADNEW jane: modify gas atmosphere gradient
      ...
      if(FIELDTYPE==FIELDJONMAD){
        ...
        pr[RHO]=RADNT_RHOATMMIN*pow(r/RADNT_ROUT,-1.1); // SUPERMADNEW: jane: change the index back to 1.1
      }
    }
    else{
